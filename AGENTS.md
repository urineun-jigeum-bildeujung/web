# 골라주개냥 Agent Guide (Karpathy Guidelines)

이 문서는 AI 에이전트가 **골라주개냥** 프로젝트에서 작업할 때 준수해야 하는 운영 및 행동 지침입니다. Andrej Karpathy의 관찰을 바탕으로 AI 코딩 에이전트가 자주 저지르는 실수를 방지하기 위한 핵심 원칙을 통합하여 작성되었습니다.
...
- **서비스명**: 골라주개냥 — 고민은 줄이고, 우리 애한테 맞게 골라주개냥
...
- 형식은 `유형(#이슈번호): 내용`이고 한글로 씁니다. **커밋 메시지는 종결어미 없이 명사형으로 끝내십시오** — `feat(#12): 구독 상세 화면 추가`. 문서 종류별 어투는 [git-convention](./docs/conventions/git-convention.md)의 "문체" 절을 따릅니다.
...
- **모바일 우선(Mobile-First)**: 모든 스타일은 모바일 뷰를 기본으로 작성합니다. 시안은 모바일 393×852 · 태블릿 768×1024 · 웹 1920×1080으로 오지만 **전환점은 시안 폭이 아니라 `md`(768px)·`lg`(1024px)입니다.** 상세는 [design-convention](./docs/conventions/design-convention.md)의 "시안 기준 크기" 절을 따릅니다.
...
| URL 쿼리 (nuqs) | 새로고침·뒤로가기에서 살아남아야 하는 화면 상태 | 필터, 정렬, 탭, 페이지, 검색어, **온보딩 단계** |
| Zustand | 그 순간에만 유효한 UI 상태 | 모달 열림 여부, 입력 중인 값 |

**서버 데이터를 Zustand에 복사하지 마십시오.** 복사하는 순간 캐시 무효화가 깨집니다.

**필터·정렬을 컴포넌트 상태로 들지 마십시오.** 목록에서 상세로 갔다 돌아오면 조건이 사라지고, 그 화면이 클라이언트 컴포넌트로 굳어 5.2절의 "상품 목록은 서버 컴포넌트 우선"과 어긋납니다. 판단 기준과 근거는 [srp-convention](./docs/conventions/srp-convention.md)의 "상태를 어디에 두는가"를 따릅니다.

**URL 쿼리에는 세 가지 기본값을 확인하십시오.** 셋 다 놓치면 빌드나 실사용에서만 드러납니다.

| 옵션 | 기본값 | 언제 바꾸나 |
| --- | --- | --- |
| `history` | `replace` | 탭·단계처럼 **화면 구성이 바뀌는 값**은 `push`. 아니면 뒤로가기가 그 전환을 건너뛰고 화면을 떠납니다 |
| 파서 | — | 보기가 정해진 값은 `parseAsStringLiteral`. `parseAsString`은 검증하지 않아 목록이 통째로 빕니다 |
| `Suspense` | — | `useQueryState`를 쓰는 라우트는 감싸야 합니다. 빌드에서만 실패합니다 |
...
shadcn은 라이브러리가 아니라 코드가 저장소에 복사되는 방식입니다. **그래서 `devDependencies`에 있습니다** — 런타임 코드가 `shadcn`을 import하지 않고, `dependencies`에 두면 CLI가 딸고 오는 express·MCP SDK까지 배포 의존성에 들어갑니다 (#332). 되돌리지 마십시오. `src/shared/ui/` 하위 파일을 임의로 수정하면 내장된 접근성이 조용히 깨질 수 있습니다. 수정이 필요하면 리뷰를 거치십시오.
