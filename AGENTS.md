...
- **PR을 올릴 때 `CHANGELOG.md`의 `[Unreleased]`에 한 줄 기록하십시오.** 커밋 메시지 복사가 아니라 사람이 읽을 문장으로 쓰고 `(#이슈번호)`를 붙입니다. 상세는 [git-convention](./docs/conventions/git-convention.md)의 "변경 이력 기록"을 따릅니다.
