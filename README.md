...
디자인 감시부터 구현까지 커맨드로 이어집니다.

```
/loop 30m /figma-watch <파일 URL>
  → 첫 실행: 프레임 전체를 구현 여부와 대조해 미구현 백로그 현황판 출력
  → 이후 30분마다: 새 프레임·삭제·내부 구조 변경 감지
       ├─ 미구현 새 프레임 → 자동 분석 → 배치 계획 승인 게이트에서 대기
       └─ 구현된 화면의 디자인 변경 → 코드 갱신 필요 신호

/from-figma <번호·URL>   ← 화면 구현은 항상 여기서 시작 (백로그 번호 또는 프레임 URL)
  → 분석 → 배치 승인 게이트 → 이슈·브랜치 → 구현 → 검증 → 커밋 → /pr
```
