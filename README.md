...
화면에서 난 오류와 웹 바이탈을 인프라팀의 Grafana로 보내고, **같은 출처 API 요청에만** 추적 헤더(`traceparent`)를 실어 백엔드 로그와 잇습니다. 수집 주소와 키가 있는 환경에서만 켜지고, 페이지의 첫 리소스를 다 받은 뒤에 따로 불러와 초기 로딩을 늦추지 않습니다.
