...
| 구분 | 패키지 | 버전 |
| --- | --- | --- |
| 결제 UI·결제창 | @tosspayments/tosspayments-sdk | 2.8.1 |

결제 UI와 결제창까지가 이 저장소의 몫이고 **결제 승인은 백엔드가 맡습니다.** 승인 API는 시크릿 키로 인증하는데 그 키가 브라우저에 닿으면 누구나 결제를 승인할 수 있기 때문입니다.

### 푸시

| 구분 | 패키지 | 버전 |
| --- | --- | --- |
| 웹 푸시 토큰 발급·수신 | firebase (`app`·`messaging`만) | 12.19.0 |

브라우저에서 FCM 토큰을 받아 서버에 알리고, 탭이 보이는 동안 온 푸시를 받는 데까지가 이 저장소의 몫이고 **발송은 백엔드가 맡습니다.** 클라이언트 설정값(`NEXT_PUBLIC_FIREBASE_*`)은 노출되는 것이 정상이고 서버 키는 백엔드가 쥡니다.

### 관측

| 구분 | 패키지 | 버전 |
| --- | --- | --- |
| 오류·성능 수집 | @grafana/faro-web-sdk | 2.12.1 |
| 백엔드 추적 연결 | @grafana/faro-web-tracing | 2.12.1 |
| 서버 지표 | prom-client | 15.1.3 |

화면에서 난 오류와 웹 바이탈을 인프라팀의 Grafana로 보내고, **같은 출처 API 요청에만** 추적 헤더(`traceparent`)를 실어 백엔드 로그와 잇습니다. 수집 주소와 키가 있는 환경에서만 켜지고, 페이지의 첫 리소스를 다 받은 뒤에 따로 불러와 초기 로딩을 늦추지 않습니다. 서버 쪽 메모리·CPU·이벤트 루프 지연은 `/metrics`로 인프라의 Prometheus가 읽어 가며, 바깥에서는 이 주소가 열리지 않습니다.
