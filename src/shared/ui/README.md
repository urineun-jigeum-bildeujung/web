...
| `icon/` | 디자인 시스템 아이콘 53종. `name`으로 고르고 색은 글자색, 크기는 `size-*`. `icon-shapes.ts`는 Figma에서 스크립트로 만든 파일이라 손으로 고치지 않는다 |
...
| `single-input-screen/` | 한 가지만 묻고 하단 완료로 끝내는 화면 골격. `submitting`을 주면 완료 버튼이 잠기고 대기 중임을 알린다 — 이 골격을 쓰는 화면이 여섯이라 각자 감싸지 않게 여기서 `LoadingSwap`을 두른다 |
