...
| 파일 | 설명 |
| --- | --- |
| `client.ts` | 공통 fetch 래퍼(`apiRequest`)와 `ApiError` — base URL·헤더 조립을 한 곳으로 모음 |
| `client.test.ts` | `apiRequest` 단위 테스트 |

- base URL은 `NEXT_PUBLIC_API_BASE_URL`을 읽고, 없으면 `/api/v1`을 쓴다.
- 인증(JWT) 헤더는 방식 확정 후 `client.ts`의 `buildHeaders`에만 추가한다. 에러 응답 본문 파싱과 토큰 재발급 흐름도 백엔드 규격 확정 후 여기에 붙인다.
