...
기능정의서와 유즈케이스는 취소도 "유형=취소"로 이 화면에 오게 적어 두었지만, **시안과 IA는 취소 확인창에서 끝낸다.** 그 확인창은 처음에 목록에 있다가 주문 상세 맨 아래로 옮겼다(#410). 백엔드도 그 기준으로 `POST /orders/{orderId}/cancel`에 요청 본문을 두지 않았다(2026-09-21 회신).
