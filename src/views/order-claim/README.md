...
| `model/claim-reasons.ts` | 사유 보기 다섯. 문구는 시안 그대로 |
...
| `model/to-claim-request.ts` | 모은 값을 접수 요청으로. 서버가 받지 않는 값은 사유 글에 묶는다 |
| `model/to-claim-request.test.ts` | 줄마다 빠짐없이 실리는지, 서버 한도 1,000자 안에 드는지 |
| `index.ts` | 공개 API |
...
## 서버가 받지 않는 값은 사유 글에 묶는다

`CreateClaimRequest`는 `claimType`·`reason`(자유 문자열)·`items`·`imageUrls`뿐인데 시안은 사유 보기·상세 사유·수거 희망일·수거 요청사항을 받는다. 백엔드가 바빠 필드를 늘리기 어려워(2026-09-23) 줄마다 머리글을 달아 `reason`에 싣는다. 비어 있는 선택 항목은 줄째 뺀다.
...
**백엔드는 사유 코드 필드 하나만 더하고, 수거 희망일·요청사항 필드는 만들지 않기로 했다(2026-09-23).** 그래서 이 묶음은 그대로 간다.

**사유 코드는 보기 다섯의 이름(`CHANGE_OF_MIND`·`DAMAGED`·`WRONG_ITEM`·`NOT_AS_DESCRIBED`·`OTHER`)을 백엔드가 그대로 받기로 했다.** 맞지 않는 값은 서버가 오류로 돌린다. 필드 이름이 정해지면 `to-claim-request`에서 한 줄 더 싣는다 — 지금은 문구만 사유 글 첫 줄로 간다.
...
| `MYPA_261` | 이번 구현 | 까닭 |
| --- | --- | --- |
| 사유 라디오 넷 | 시안의 보기 다섯 | 시안이 "상품 설명과 달라요"를 더했다. 서버에는 문구가 사유 글 첫 줄로 간다 |
