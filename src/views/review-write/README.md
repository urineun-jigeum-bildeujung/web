...
| 파일 | 설명 |
| --- | --- |
| `ui/review-write-view.tsx` | 두 단계 조립과 완료 화면. 1단계 별점·사용 기간·반응 5문항, 2단계 요약·아이(여러 마리)·급여 편의성·사진·후기. |
...
**필수는 별점 · 사용 기간 · 아이 · 후기 글과 반응 문항 둘(기호성·급여 편의성)이다.** 와이어프레임은 반응 3문항을 필수로 묶었는데 시안이 둘만 필수로 정했다(#302). 모르는 항목까지 아무 답이나 고르게 하면 추천 근거가 흐려진다. 어느 아이가 먹었는지는 여전히 필수다 — 아이를 모르면 그 답을 다음 추천에 쓸 수 없다. **아이는 여러 마리를 고른다**(#391) — 한 상품을 두 아이에게 함께 먹이는 일이 흔한데 리뷰는 회원+상품당 한 건이라, 한 마리만 실으면 나머지의 반응이 사라진다. 요청은 `petIds` 배열이고 한 마리 이상이면 된다. 옛 초안의 `petId` 하나는 배열로 읽는다. 서버가 요구하는 `answerValues @NotEmpty`(하나 이상)는 이 둘이 채운다.
...
**작성 화면으로 들어오는 경로는 나의 상품 후기의 "작성 가능한 리뷰" 탭이다.** 그 목록은 `GET /reviews/writable`을 받는다(#349). 상품 상세에서 바로 쓰는 진입은 아직 없다.
