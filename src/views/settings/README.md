...
## 알림 스위치는 "이 기기의 푸시 허용"이다

켜면 브라우저 알림 권한을 묻고 FCM 토큰을 받아 `POST /notifications/fcm-tokens`로 알린다. 끄면 브라우저에서 토큰을 지운다(#354). 서버 구독(`subscriptions/{category}`)은 쓰지 않는다 — 카테고리가 `TIME_DEAL` 하나라 "알림설정"과 뜻이 안 맞고, 하위 화면 시안이 오면 다시 본다.

**켜짐 = 권한 허용 + 이 기기에서 켰다는 표시(`localStorage`).** 권한은 브라우저가 쥐고 있어 "껐다"를 따로 남겨야 하고, 브라우저 설정에서 권한을 거두면 표시가 남아 있어도 꺼짐으로 보인다. 서버 렌더에서는 늘 꺼짐으로 그리고 클라이언트가 `useSyncExternalStore`로 실제 값을 읽는다 — `useState` 초기값으로 읽으면 하이드레이션이 어긋난다.
