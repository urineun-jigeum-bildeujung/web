...
export function useEditPet() {
  ...
  const [petId] = useQueryState("petId");
  const { pet, isLoading, error } = useQueryPetDetail(petId ?? undefined);
  const { updatePet, isSaving } = useMutateUpdatePet(petId ?? undefined);
