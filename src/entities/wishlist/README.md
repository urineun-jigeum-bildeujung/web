...
**PATCH는 삭제가 아니라 토글이다.** 응답의 `wished`를 그대로 믿지 않는다 — 중복 요청이나 오래된 상태에서 `true`가 돌아올 수 있다. `useMutateWishlist`는 낙관적으로 먼저 빼고, 실패하면 그 상품만 원래 있던 캐시·위치로 되돌린다(다른 상품의 성공한 변경은 건드리지 않는다). 끝나면 무조건 전체·카테고리별 캐시를 무효화해 서버 상태로 재동기화하고, 재시도는 쓰지 않는다.
