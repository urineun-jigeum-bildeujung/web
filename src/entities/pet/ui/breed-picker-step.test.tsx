...
test("줄을 누르면 고른 품종과 그 종을 함께 넘긴다", () => {
  ...
  render(<BreedPickerStep value="" onConfirm={onConfirm} onCancel={vi.fn()} />);
...
test("검색창에 치면 목록이 걸러지고 지우면 돌아온다", () => {
  render(<BreedPickerStep value="" onConfirm={vi.fn()} onCancel={vi.fn()} />);
...
test("머리말의 뒤로가기가 부르던 화면으로 돌려보낸다", () => {
  ...
  render(<BreedPickerStep value="" onConfirm={vi.fn()} onCancel={onCancel} />);
