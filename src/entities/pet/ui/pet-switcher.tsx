...
export function PetSwitcher({ pets, selectedId, onSelect, onAdd, className }: PetSwitcherProps) {
  return (
    <div
      role="radiogroup"
      aria-label="아이 고르기"
    ...
    >
      {pets.map((pet) => {
        ...
        return (
          <button
            ...
            className={cn(
              CIRCLE,
              "bg-muted transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none",
      ...
      {onAdd && (
        <button
          ...
          className={cn(
            CIRCLE,
            "flex items-center justify-center border-2 border-dashed border-border text-muted-foreground",
