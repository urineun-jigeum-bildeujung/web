...
| `ui/pet-switcher.test.tsx` | 단일 선택은 라디오, 다중 선택은 체크박스로 읽히고 각각 고른 아이를 알리는지 |
...
**체구 선택지는 소형·중형·대형 셋이고 강아지에게만 묻는다.** 고양이는 체구를 묻지 않고 서버도 `null`로 둔다(PD 결정 2026-09-22, #391). 몇 kg으로 가르는지는 `SIZE_GUIDE`(초소형견 4kg 미만 · 소형견 4~10kg · 중형견 10~25kg · 대형견 25kg 이상)에 있고 `SizeGuide` 말풍선이 보인다. 초소형견은 안내에만 있어 4kg 미만은 소형을 고른다.
...
**상세 조회도 알레르기를 `{ code, displayName }`으로 준다.** 등록 선택지(`GET /pets/health-options`)와 같은 모양이라 저장은 코드로 하고 화면에는 표시명을 보인다. 한때 상세가 코드만 줘서 선택지를 따로 받아 짝을 맞추거나 코드를 그대로 보여야 했는데, 백엔드가 표시명을 함께 싣기로 하면서 사라진 문제다(#267). `healthConcerns`는 코드 자리에 한글이 들어 있어 그대로 쓴다.
