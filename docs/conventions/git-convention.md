...
**커밋 메시지와 사람이 읽는 문서는 존댓말(`~합니다`)로 쓴다.** 영어권 커밋은 명령형(`Add`, `Fix`)을 쓰지만 한국어에 그대로 옮기면 어색하고, 팀이 존댓말을 쓰기로 정했다.
...
**기록·규정 문서가 평서체인 것은 읽는 상대를 상정하지 않기 때문이다.** CHANGELOG는 무슨 일이 있었는지 남기는 기록이고 컨벤션은 규칙이라, 설명하는 글이 아니라 적어두는 글이다. 반면 README는 저장소를 처음 여는 사람에게 건네는 소개 문서다.
