...
**커밋 메시지는 명사형으로 끝낸다.** 종결어미(`~합니다`·`~한다`)를 쓰지 않는다.

```
feat(#12): 구독 상세 화면 추가
fix(#5): 본문 없는 커밋 메시지 거부하도록 수정
docs(#41): 문서별 문체 규칙 명문화
```

커밋 로그는 사람에게 말을 거는 글이 아니라 무엇을 했는지 훑는 목록이다. 종결어미를 떼면 같은 폭에 내용이 더 들어가고 훑기도 쉽다. `~함`·`~ 완료`로 끝내도 된다.
...
| README | `~합니다` |
| CHANGELOG | `~했다` |
| 컨벤션 문서 (`docs/conventions/`) | `~한다` |
| `AGENTS.md` | `~하십시오` |

**README만 존댓말인 것은 저장소를 처음 여는 사람에게 건네는 소개 문서이기 때문이다.** CHANGELOG와 컨벤션은 읽는 상대를 상정하지 않는다. 무슨 일이 있었는지 남기는 기록과 지켜야 할 규칙이라, 설명하는 글이 아니라 적어두는 글이다. `AGENTS.md`는 에이전트에게 주는 지시라 명령형(`~하십시오`)을 쓴다.

**이미 올라간 커밋은 고치지 않는다.** 어투를 맞추려고 이력을 되감는 것은 "rebase에서 꼬이는 지점" 절이 금지하는 일이다. 새 커밋부터 적용한다.

## 커밋

- 커밋 메시지는 **한글**이고 명사형으로 끝낸다. 위 "문체" 절을 따른다.
...
- **commit-msg** — 메시지가 `유형(#이슈번호): 내용` 형식(이슈 없으면 `유형: 내용`)인지 검사한다. 이슈 번호는 1 이상만 받으므로 실재하지 않는 `#0`을 손으로 적으면 막힌다. Merge·Revert와 autosquash(`fixup!`·`squash!`·`amend!`) 커밋은 Git이 만드는 형식이라 통과한다.
...
PR에 붙는 라벨이 곧 릴리스 노트의 분류가 된다. **라벨은 PR 제목에서 자동으로 부착되므로 직접 달 필요가 없다.** `.github/workflows/pr-label.yml`이 제목 앞의 유형(`feat`·`fix`·`refactor`·`style`·`test`·`chore`·`docs`·`design`)을 읽어 같은 이름의 라벨을 붙이고, 제목을 고치면 라벨도 따라 바뀐다. 제목을 형식에 안 맞게 고치면 이전 라벨을 떼기만 한다.

즉 **PR 제목을 커밋 형식대로 쓰는 것만으로 릴리스 노트 분류까지 끝난다.** 제목이 형식에 안 맞으면 라벨이 없는 채로 남아 릴리스 노트에서 "그 밖의 변경"으로 떨어진다.
