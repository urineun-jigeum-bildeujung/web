...
**커밋 메시지와 README는 존댓말(`~합니다`)로 쓴다.** 영어권 커밋은 명령형(`Add`, `Fix`)을 쓰지만 한국어에 그대로 옮기면 어색하고, 팀이 존댓말을 쓰기로 정했다.
...
**CHANGELOG와 컨벤션 문서가 평서체인 것은 읽는 상대를 상정하지 않기 때문이다.** 무슨 일이 있었는지 남기는 기록과 지켜야 할 규칙이라, 설명하는 글이 아니라 적어두는 글이다. 반면 README는 저장소를 처음 여는 사람에게 건네는 소개 문서다. `AGENTS.md`는 에이전트에게 주는 지시라 명령형(`~하십시오`)을 쓴다.

**이미 올라간 커밋은 고치지 않는다.** 어투를 맞추려고 이력을 되감는 것은 "rebase에서 꼬이는 지점" 절이 금지하는 일이다. 새 커밋부터 적용한다.

## 커밋

- 커밋 메시지는 **한글 존댓말**. 위 "문체" 절을 따른다.
- 형식: `유형(#이슈번호): 내용` — 예: `feat(#12): 구독 상세 화면을 추가합니다`.
