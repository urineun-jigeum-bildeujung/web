...
- **탭 대상 크기는 디자인 시스템이 정한다.** Figma `골라주개냥 Design System`이 `button/m`(32px)·
  `button/xl`(44px)처럼 크기 토큰을 갖고 있으므로, 시안이 지정한 값을 그대로 쓴다.
  접근성을 이유로 시안보다 키우지 않는다 — 화면마다 크기가 달라지면 그게 더 큰 문제다.
- **시안에 없는 자리는 44×44를 기본으로 한다.** 개발용 화면이나 시안이 아직 없는 컴포넌트가 그렇다.
- 시안 값이 44보다 작아 누르기 어려워 보이면 **임의로 키우지 말고 디자인팀에 확인한다.**
  WCAG는 24×24(AA)와 44×44(AAA) 두 기준을 두고 있어, 작다고 해서 곧바로 위반은 아니다.
