...
`npm run knip`이 미사용 파일·export·의존성을 찾는다. **CI가 PR에서 실행하므로 PR을 올리기 전에 한 번 돌려 본다.**

로컬 훅으로 막지 않는 이유는 FSD가 아래층부터 쌓아 올라가는 구조이기 때문이다. `entities` 슬라이스를 만들고 아직 `views`에 조립하지 않은 중간 상태는 정상이며, 이때 push가 막히면 작업 백업을 못 한다. 죽은 코드가 문제가 되는 시점은 `dev`에 병합될 때다.
