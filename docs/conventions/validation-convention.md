...
### e2e는 관련 spec만 골라 돌리지 않는다

**화면에서 요소를 지우거나 이름·라벨을 바꿨으면 `npm run test:e2e`를 통째로 돌린다.** 바꾼 화면의 spec만 돌리면 통과하고 CI에서 깨진다.

지운 요소를 다른 spec이 **어떤 이름으로 붙잡고 있는지 grep으로 다 찾지 못하기 때문**이다. 실제로 온보딩 닫기 버튼을 지우면서 `onboarding-*.spec.ts`만 돌려 통과했는데, `screens.spec.ts`가 그 버튼을 `aria-label="닫기"`로 눌러 이탈 모달을 열고 있었다. 지운 쪽 코드에는 그 문자열이 없어 검색에 걸리지 않았다.

곧바로 다음 작업에서 값을 했다. 품종 표기를 `코리안 숏헤어 (코숏)`에서 `코리안 숏헤어`로 바꿨을 때 같은 spec이 옛 이름을 잡고 있었고, 전체를 돌려 로컬에서 잡았다.

| 바꾼 것 | 어떻게 돌리나 |
| --- | --- |
| 요소·문구·라벨을 지우거나 이름 변경 | **전체** |
| 라우트·핵심 인터랙션 추가·변경 | 전체 |
| 한 화면 안의 스타일·여백 | 생략 가능 |
...
**자동 검사 도구를 두지 않는다.** FSD는 아래층부터 쌓아 올라가므로 슬라이스를 만들고 아직 조립하지 않은 중간 상태가 정상이고, 그걸 죽은 코드로 잡는 도구는 오탐이 잦다.

대신 코드 리뷰에서 본다. 무언가를 지우거나 슬라이스를 옮겼으면 남은 참조가 없는지 직접 확인하고, 안 쓰게 된 의존성은 `package.json`에서 함께 뺀다.
