...
| `/login` | 로그인 | 비회원 | 소셜 버튼은 `{NEXT_PUBLIC_OAUTH_BASE_URL}/oauth2/authorization/{kakao\|google}`로 브라우저 이동(fetch 아님). **이 주소만 `/api/v1`이 아니라 `/api/auth`다** | 소셜 연결됨. 아이디·비밀번호 폼은 붙일 API가 없어 시안대로 둠(#184) |
| `/auth/callback?code=` · `?error=` | 소셜 인증 복귀 | 비회원 | 성공은 `code`(일회용, 60초 만료), 실패는 `error=login_failed`. `POST /auths/token/exchange`로 교환해 토큰 저장 후 **`needsSignup`**이면 `/signup?nickname=`, 아니면 `/`로 `replace` | 구현됨 |
| `/signup?step=&nickname=` | 회원가입 | 가입을 마치지 않은 사용자 | `step=terms\|nickname`, `nickname`은 교환 응답의 추천 닉네임. 약관 동의는 소셜 가입도 자체 화면에서 받는다 | 구현됨. 마치면 새 토큰으로 교체하고 `/onboarding`으로 `replace` |
...
| `/likes` | 좋아요 목록 | 인증 필요 후보 | `tab=liked\|recent\|often`, `notice=all\|new\|seen`(찜 탭 전용) | 찜 탭만 UI 시안 반영(#274), 나머지 두 탭은 와이어프레임 |
...
| 주문별 외부 문의 | 외부 페이지에 주문 식별자를 전달할 수 있는지 미정 | PM·연동 담당 |
